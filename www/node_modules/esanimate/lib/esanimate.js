...
esanimate.astify = function (obj, canonicalize) {
    var type = typeof obj;
    if (obj === null || type === 'boolean' || type === 'number' || type === 'string' || Object.prototype.toString.call(obj) === '[object RegExp]') {
        ...
        return {
            ...
            properties: keys.map(function (key) {
                return {
                    kind: 'init',
                    key: /^\w+$/.test(key) ? { type: 'Identifier', name: key } : { type: 'Literal', value: key },
                    value: esanimate.astify(obj[key], canonicalize)
                };
            })
        };
    } else if (typeof obj === 'function') {
        var functionAst = require('esprima').parse('!' + obj.toString()).body[0].expression.argument;
...
esanimate.objectify = function (ast) {
    if (ast.type === 'Literal') {
        ...
        ast.properties.forEach(function (property) {
            ...
            obj[property.key.name || property.key.value] = esanimate.objectify(property.value);
        });
        return obj;
        ...
        /*jshint evil:true*/
        var fn = new Function(ast.params.map(function (param) {
            return param.name;
        }).join(","), require('escodegen').generate({ type: 'Program', body: ast.body.body }));
        /*jshint evil:false*/
        if (ast.id) {
        ...
        throw new Error("esanimate.objectify: Cannot convert " + JSON.stringify(ast));
    }
};
